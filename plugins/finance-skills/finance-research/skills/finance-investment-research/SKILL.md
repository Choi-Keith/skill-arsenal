...
> **数据源规范**：参见 `../finance-financial-data/SKILL.md`。所有财务数据必须来自两个独立来源，误差>1%须标记。
...
数据收集完成后，**必须调用 `scripts/financial_rigor.py` 对关键数据进行程序化验证**，杜绝LLM心算误差。
...
```bash
python3 scripts/financial_rigor.py three-scenario \
...
**Step 2 — 取数核验：**
对清单中每个数据点，按 `../finance-financial-data/SKILL.md` 规范从可靠信源取数
